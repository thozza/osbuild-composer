...
func (s *Server) RequeueOrFinishJob(token uuid.UUID, maxRetries uint64, result json.RawMessage) error {
	...
	case JobTypeBootcInfoResolve:
		...
		jobResult = &bootcInfoResolveJR.JobResult
		// NOTE: The arch is not stored in the job result, so we need to get it from the job args.
		var bootcInfoResolveJob BootcInfoResolveJob
		_, rawArgs, _, _, err := s.jobs.Job(jobId)
		if err != nil {
			logrus.Errorf("Error getting bootc info resolve job args: %v", err)
		} else {
			if unmarshalErr := json.Unmarshal(rawArgs, &bootcInfoResolveJob); unmarshalErr == nil {
				arch = bootcInfoResolveJob.Arch
			} else {
				logrus.Errorf("Error unmarshalling bootc info resolve job args: %v", unmarshalErr)
			}
		}
	case JobTypeBootcPreManifest:
		var bootcPreManifestJR BootcPreManifestJobResult

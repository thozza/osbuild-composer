...
func (impl *BootcInfoResolveJobImpl) Run(job worker.Job) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to resolve bootc info for ref %q: %s", args.Ref, err.Error())
	}
	...
	if err != nil {
		...
		result.JobError = clienterrors.New(clienterrors.ErrorBootcInfoResolve, reason, nil)
		return fmt.Errorf("failed to convert bootc info to DTO for ref %q: %s", args.Ref, err.Error())
	}
